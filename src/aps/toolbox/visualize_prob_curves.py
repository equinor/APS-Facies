...
def plot_probability_logs(
    ...
    for m, well_name in enumerate(well_list):
        ...
        try:
            ...
            raise IOError(
                f'Well:{well_name} may not be blocked before running this script?'
            )
        ...
        for n in range(nlogs):
            ...
            axs[plot_number_prob_logs].fill_betweenx(
                z_values, cumulative_prob_refined[n, :], label=logname, zorder=-n
            )
...
def cumulative_prob_logs(
    ...
    for j in range(num_val):
        for i in range(refinement_factor):
            ...
            if zone_log_val[j] >= 0.0:
                zone_log_val_refined[indx] = zone_log_val[j]
            else:
                zone_log_val_refined[indx] = 0
            indx += 1
