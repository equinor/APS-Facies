...
# Note: If APS cannot find any file with updated field parameter
# APS will instead load the initial ensemble realization of the field parameter. This makes it possible to
# specify in ERT that only a subsert of all field parameters created by APS in initial ensemble is updated by ERT.
#
...
# Note that also when updating residual fields and trends separately as described above, it is possible that
# ERT is configured not to update all the residual fields, but only a subset. The fields that are not updated
# will be read from the directory where the initial ensemble realization is stored instead.
...
def _load_field_values_roff(field_name, path, grid=None, debug_level=Debug.OFF):
    if debug_level >= Debug.VERY_VERBOSE:
        ...
        print('--- Format: ROFF')
    try:
        property = xtgeo.gridproperty_from_file(path, fformat='roff', name=field_name)
    except ValueError:
        print(
            f'Error: Check that filename is correct for field parameter name: {field_name} '
            'in ERT keyword FIELD'
        )
        raise
    return property.values
...
def field_name_from_full_name(full_field_name, zone_name, region_name=''):
    ...
    if len(region_name) == 0:
        ...
        field_name, is_trimmed = _trim(
            field_name_tmp, prefix=zone_name + '_' + region_name + '_'
        )
    if not is_trimmed:
        raise IOError(f'Unexpected name of field to import: {full_field_name}')
    return field_name
...
def run(
    ...
    **kwargs,
    ...
    # ERT iteration number and realization number
    ert_iteration = kwargs.get('ert_iteration')
    ...
    if geo_grid_name is None:
        raise ValueError('Missing geo grid name as input to import of GRF from ERT')
    fmu_mode = kwargs.get('fmu_mode', False)
    if not fmu_mode:
        raise ValueError('The import of GRF is only available in FMU mode with AHM')
    ...
    if import_from_ert:
        print(
            f'Import updated field parameters from ERT into {fmu_grid_name} and {geo_grid_name}'
        )
    else:
        print(
            f'Initial ensemble field parameters generated by APS is loaded into {fmu_grid_name} and {geo_grid_name}'
        )

    if debug_level >= Debug.VERBOSE:
        print(f'-- Path for field parameters relative to RMS project: {load_dir}')

    # Geomodel grid
    geo_grid_model = project.grid_models[geo_grid_name]
    if geo_grid_model.is_empty(project.current_realisation):
        raise ValueError(
            f'Grid model for geo grid: {geo_grid_name} '
    ...
    if fmu_grid_model.is_empty(project.current_realisation):
        raise ValueError(
            f'Grid model for ERTBOX grid: {fmu_grid_name} '
            f'is empty for realization {project.current_realisation}.'
        )
    ...
    if not use_residuals:
        import_and_update_ertbox_and_geogrid(
            project,
            aps_model,
            fmu_grid_model,
            geo_grid_model,
        ...
        import_and_update_ertbox_and_geogrid_with_residuals(
            project,
            aps_model,
            fmu_grid_model,
            geo_grid_model,
...
def import_and_update_ertbox_and_geogrid(
    project,
    aps_model: APSModel,
    fmu_grid_model,
    geo_grid_model,
    ...
    for zone in aps_model.zone_models:
        if aps_model.isSelected(zone.zone_number, zone.region_number):
            ...
            for full_field_name in zone.gaussian_fields_in_truncation_rule:
                ...
                if ert_iteration > 0:
                    # Running in ERT iteration > 0
                    # Expect to read updated fields
                    # but if that does not exist read initial ensemble field instead
                    if field_location.exists():
                        # Read values into fmu grid (ERTBOX)
                        print(f'Load updated ensemble realization for {field_name}')
                        ...
                        # No update found on ERT run path
                        load_dir_init = Path(APSConfig.aps_initial_ensemble_path())
                        print(
                            f'WARNING: No update is found for {field_name}. '
                            'Load initial ensemble realization'
                        )
                        field_location = (
                            load_dir_init / f'{full_field_name}.{file_format}'
                        )
                        # Read values into fmu grid (ERTBOX)
                    ...
                    # Running in ERT iteration = 0
                    # Expect to read initial ensemble fields
                    print(f'Load initial ensemble realization for {field_name}')
                    field = load_field_values(
                        full_field_name,
                        field_location,
                        grid=xtgeo_fmu_grid,
                ...
                field_extracted = extract_values_from_ertbox_grid_to_geogrid_simbox(
                    field, zone.grid_layout.value, nz_layers
                )
            ...
            if debug_level >= Debug.VERY_VERBOSE:
                for name in parameter_names_geo_grid:
                    if aps_model.use_regions:
                        print(
                            f'--- Update parameter {name} for (zone number, region number) = ({zone.zone_number},{zone.region_number})  in {geo_grid_model.name}'
                        )
                    else:
                        print(
...
def import_and_update_ertbox_and_geogrid_with_residuals(
    project,
    aps_model: APSModel,
    fmu_grid_model,
    geo_grid_model,
    ...
    use_residuals = True
    (nx_ertbox, ny_ertbox, nz_ertbox) = fmu_grid_model.get_grid(
        project.current_realisation
    ).simbox_indexer.dimensions
        ...
        if aps_model.isSelected(zone.zone_number, 0):
            ...
            for full_field_name in zone.gaussian_fields_in_truncation_rule:
                ...
                if zone.hasTrendModel(full_field_name):
                    ...
                    field_location = (
                        load_dir / f'{full_field_name_residual}.{file_format}'
                    )
                ...
                if ert_iteration > 0:
                    # Expect to find field parameter updated by ERT.
                    # If it does not exist, use the initial ensemble realization of it instead.
                    if field_location.exists():
                        # Read values into fmu grid (ERTBOX)
                        print(
                            f'Load updated ensemble realization for {field_name_in_file}'
                        )
                        field = load_field_values(
                            field_name_in_file,
                            field_location,
                        ...
                        # No update found on ERT run path
                        print(
                            f'WARNING: No update is found for {field_name_in_file}. '
                            'Load initial ensemble realization'
                        )
                        load_dir_init = Path(APSConfig.aps_initial_ensemble_path())
                        if zone.hasTrendModel(full_field_name):
                            ...
                            field_location = (
                                load_dir_init
                                / f'{full_field_name_residual}.{file_format}'
                            )
                            field_name_in_file = full_field_name_residual
                        else:
                            field_location = (
            ...
            for full_field_name in zone.gaussian_fields_in_truncation_rule:
                field_ertbox_1D = getContinuous3DParameterValues(
                    fmu_grid_model,
                    full_field_name,
                ...
                field_ertbox_3D = np.reshape(
                    field_ertbox_1D, (nx_ertbox, ny_ertbox, nz_ertbox)
                )
                if handedness == Direction.right:
                    field_ertbox_3D_flip = flip_grid_index_origo(
                        field_ertbox_3D, ny_ertbox
                    )
                    field_ertbox_3D = field_ertbox_3D_flip
                field_extracted = extract_values_from_ertbox_grid_to_geogrid_simbox(
                    field_ertbox_3D, zone.grid_layout.value, nz_layers
                )
                parameter_values_geo_grid.append(field_extracted)
