#!/bin/env python
# This script use both nrlib and ROXAR API functions and run simulations sequentially and not in parallell
...
import src.generalFunctionsUsingRoxAPI 

importlib.reload(src.APSModel)
importlib.reload(src.APSZoneModel)
importlib.reload(src.generalFunctionsUsingRoxAPI)
...
from src.generalFunctionsUsingRoxAPI import  setContinuous3DParameterValuesInZoneRegion, getGridAttributes
    ...
    for key, zoneModel in allZoneModels.items():
        ...
        for i in range(len(gaussFieldNames)):
            ...
            if variogramName == 'general_exponential':
                simVariogram = nrlib.variogram(variogramName, mainRange, perpRange, vertRange, 
                                               azimuthInNRLIB, dip, power)
            else:
                simVariogram = nrlib.variogram(variogramName, mainRange, perpRange, vertRange, 
                                               azimuthInNRLIB, dip)
