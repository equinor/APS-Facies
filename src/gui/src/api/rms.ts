...
export default {
  ...
  // @ts-ignore
  chooseFile: (mode: string, filter: string, suggestion: string = ''): Promise<string> => typeof rms !== 'undefined' ? rms.chooseFile(mode, filter, suggestion) : new Promise((resolve, reject) => resolve(null)),
}
