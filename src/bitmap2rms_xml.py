...
#             where the necessary environment varibles are set automatically.
...
#       Coordinates   -  specify coordinates for corner points of a rectangle in the map. (xmin,ymin) corresponds to pixel (Imin,Jmin)
#                        (xmax,ymax) corresponds to pixel (Imax,Jmax)  where Imin,Imax,Jmin,Jmax is specified in keyword PixelInterval.
...
#                        Hence the user will have to identify the corner point pixels (Imin,Jmin), (Imax,Jmax) and their corresponding
#                        geographic (UTM coordinates) (xmin,ymin) and (xmax,ymax).
...
import sys
import copy
import ConvertBitMapToRMS

# -------------  Main ----------------------
if __name__ == "__main__":
    modelFileName = 'bitmap2rms_model.xml'
    defineFaciesCode = 0
    if len(sys.argv) >= 2:
        defineFaciesCode = int(sys.argv[1])
        print('Calculate map with facies')

    functionName = 'bitmap2rms'
    testScript = 0


convertBitmap = ConvertBitMapToRMS.ConvertBitMapToRMS(modelFileName,defineFaciesCode)

convertBitmap.printContents()

convertBitmap.convert()
convertBitmap.writeFile()
