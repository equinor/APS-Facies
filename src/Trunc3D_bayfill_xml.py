...
class Trunc3D_bayfill:
    ...
    def __interpretXMLTree(self,trRuleXML,mainFaciesTable,faciesInZone,printInfo,modelFileName):
        ...
        if fpObj == None:
            print('Error in ' + self.__className)
            print('Error: Floodplain facies is not specified.')
            err = 1
        else:
            text = fpObj.text
        ...
        if sbObj == None:
            print('Error in ' + self.__className)
            print('Error: Subbay facies is not specified.')
            err = 1
        else:
            text = sbObj.text
            self.__faciesInTruncRule.append(text.strip())
        ...
        if wbfObj == None:
            print('Error in ' + self.__className)
            print('Error: Wave influenced bayfill facies (WBF) is not specified.')
            err = 1
        else:
            text = wbfObj.text
            self.__faciesInTruncRule.append(text.strip())
        ...
        if bhdObj == None:
            print('Error in ' + self.__className)
            print('Error: Bayhead delta facies (BHD) is not specified.')
            err = 1
        else:
            text = bhdObj.text
            self.__faciesInTruncRule.append(text.strip())
        ...
        if lgObj == None:
            print('Error in ' + self.__className)
            print('Error: Lagoon facies is not specified.')
            err = 1
        else:
            text = lgObj.text
            self.__faciesInTruncRule.append(text.strip())
        ...
        if SFObj == None:
            ...
            print('Warning: Truncation parameter SF is not specified. Use default = 0.0')
            self.__param_sf   = 0.0
        else:
            text = SFObj.text
        ...
        if YSFObj == None:
            print('Warning in ' + self.__className)
            print('Warning: Truncation parameter YSF is not specified. Use default = 1.0')
        ...
        if SBHDObj == None:
            print('Warning in ' + self.__className)
            print('Warning: Truncation parameter SBHD is not specified. Use default = 0.0')
        ...
        if self.__nFacies != 5:
            ...
            print('Error: Different number of facies in truncation rule and in zone.')
            err = 1

        # Check that specified facies is defined for the zone
        if not self.__checkFaciesForZone():
            print('Error when reading model file: ' + modelFileName)
            print('Error: Mismatch between facies for truncation rule and facies for the zone.')
            err = 1
            ...
            for j in range(len(self.__faciesInTruncRule)):
                ...
                for k in range(len(self.__faciesInZone)):
                    ...
                    if fN == fName:
                        ...
                        break
                if fIndx == -999:
                    print('Error in Trunc3D_Bayfill.  Programming error.')
                    sys.exit()
...
#        return
    ...
    def XMLAddElement(self,parent):
        ...
        attribute = {'name':'Trunc3D_Bayfill','nGFields':'3'}
        tag        = 'TruncationRule'
    ...
    def __setZeroPolygon(self):
        ...
        poly = []
        poly.append([0,0])
        poly.append([0,0.0001])
        poly.append([0.0001,0.0001])
        poly.append([0,0.0001])
        poly.append([0,0])
        return poly
    ...
    def setTruncRule(self,faciesProb,cellIndx=0):
        ...
        if self.__printInfo >= 3 and cellIndx == 0:
            print('Debug output: P1,P2,P3,P4,P5: ' + str(P1) + ' ' + str(P2) + ' ' + str(P3) + ' ' + str(P4) + ' ' + str(P5))
        if sbhd>0.999:
            sbhd=0.999
        elif sbhd<0.001:
            sbhd=0.001
        ...
        if sumProb == 0.0:
            print( 'Error: All input probabilites are <= 0.0')
            sys.exit()
        ...
        if P4==0:
            ...
            if Amax==0:
                ...
                if AmP4sqrt<=0.5*(XL-X2)*(XL-X2)/(c-sf): #AmP4sqrt<=A1
                    ...
                    Xm2=Xm-c*Ym
                    caseA=1
                elif AmP4sqrt<=(XL-X4)*YS-0.5*(XL-X4)*(XL-X4)/c-0.5*(X2-X4)*YS: #A1<AmP4sqrt<=A2
                    Ym=math.sqrt(max(0,((XL-X2)*(XL-X2)/(sf*sf)) + (c/(sf*(c-sf)))*((XL-X2)*(XL-X2)/c+2.0*AmP4sqrt)))-(XL-X2)/sf
                    Ym2=(1.0-sf/c)*Ym-(XL-X2)/c
                    Xm=XL
                    Xm2=X2-sf*Ym
                    caseA=2
                elif AmP4sqrt<=(XL-X4)-0.5*(X2-X4)*YS-0.5*(XL-X4)*(XL-X4)/c: #A2<AmP4sqrt<=A3
                ...
                caseT=3
                if AmP4sqrt<=0.5*c*YS*YS-0.5*(X2-X4)*YS: #AmP4sqrt<=A1
                    Ym=math.sqrt(2.0*AmP4sqrt/(c-sf))
                    Ym2=0.0
                    Xm=X2+2.0*AmP4sqrt/Ym
                    Xm2=Xm-c*Ym
                    caseA=1
                elif AmP4sqrt<=0.5*c-0.5*(X2-X4)*YS: #A1<AmP4sqrt<=A2
                    Ym=math.sqrt(2.0*(AmP4sqrt+0.5*(X2-X4)*YS)/c)
                    Ym2=0.0
                    Xm=X4+c*Ym
                    Xm2=X4
                    caseA=2
                elif AmP4sqrt<=0.5*c-0.5*(X2-X4)*YS+(XL-X4-c): #A2<AmP4sqrt<=A3
                ...
                if AmP4sqrt<=0.5*c*YS*YS-0.5*(X2-X4)*YS: #AmP4sqrt<=A1
                    ...
                    Xm2=Xm-c*Ym
                    caseA=1
                elif AmP4sqrt<=(0.5/c)*(XL-X4)*(XL-X4)-0.5*(X2-X4)*YS: #A1<AmP4sqrt<=A2
                    Ym=math.sqrt(2.0*(AmP4sqrt+0.5*(X2-X4)*YS)/c)
                    Ym2=0.0
                    Xm=X4+c*Ym
                    Xm2=X4
                    caseA=2
                elif AmP4sqrt<=(0.5/c)*(XL-X4)*(XL-X4)-0.5*(X2-X4)*YS+(XL-X4)*(1.0-(XL-X4)/c):  #A2<AmP4sqrt<=A3
                    Ym2=(AmP4sqrt+0.5*(X2-X4)*YS-0.5*(XL-X4)*(XL-X4)/c)/(XL-X4)
                    Ym=Ym2+(XL-X4)/c
                    Xm=XL
                    Xm2=X4
            ...
            if AmP4sqrt<=0.5*(XL-X4)*(YS-YL-(XL-X4)/c): #AmP4sqrt<=A1
                ...
                Xm=XL
                caseA=1
            elif AmP4sqrt<=0.5*(XL-X4)*(YS-YL-(XL-X4)/c)+(XL-X4)*(1.0-YS): #A1<AmP4sqrt<=A2
        ...
        if fssit == 1:
            ...
            polyFP.append([1.0,0.0])
            polyFP.append([X2,YF2])
            polyFP.append([X1,YF])
            polyFP.append([0.0,YF])
            polyFP.append([0.0,0.0])
        ...
        #        polygon = Polygon([(X4,YS),(X3,1.0),(X1,1.0),(X1,YF),(X2,YS)],True) #SB
            polySB.append([X4,YS])
            polySB.append([X3,1.0])
            polySB.append([X1,1.0])
            polySB.append([X1,YF])
            polySB.append([X2,YS])
            polySB.append([X4,YS])
            polygons.append(polySB)
        ...
        if bhdsit == 1:
            ...
            polyLG.append([1.0,0.0])
            polyLG.append([1.0,1.0])
            polyLG.append([XL,1.0])
            polyLG.append([XL,0.0])
            polyLG.append([1.0,0.0])
        ...
        if bhdsit < 4:
            ...
            if bhdsit == 1:
                if Xm2 > X4:
                    ...
                    polyBHD.append([Xm,Ym2])
                    polyBHD.append([Xm2,Ym])
                    polyBHD.append([X4,1.0])
                    polyBHD.append([X4,0.0])
                    polyBHD.append([Xm,0.0])
                    polyBHD.append([Xm,Ym2])
                    polygons.insert(2,polyBHD)

                    
                #                polygon = Polygon([(Xm,Ym2),(Xm2,Ym),(XL,1.0),(XL,Ym2)],True) #WBF 
                    polyWBF.append([Xm,Ym2])
                    polyWBF.append([Xm2,Ym])
                    polyWBF.append([XL,1.0])
                    polyWBF.append([XL,Ym2])
                    polyWBF.append([Xm,Ym2])
                ...
                #                polygon = Polygon([(Xm,Ym2),(Xm2,Ym),(Xm2,1.0),(XL,1.0),(XL,YL)],True) #WBF
                    polyWBF.append([Xm,Ym2])
                    polyWBF.append([Xm2,Ym])
                    polyWBF.append([Xm2,1.0])
                    polyWBF.append([XL,1.0])
                    polyWBF.append([XL,YL])
                    polyWBF.append([Xm,Ym2])
                    polygons.insert(2,polyWBF)
                ...
                if Xm2 > X4:
                    if Ym <  YS:
                        ...
                        polyBHD.append([Xm,Ym2])
                        polyBHD.append([Xm2,Ym])
                        polyBHD.append([X2,0.0])
                        polyBHD.append([XL,0.0])
                        polyBHD.append([Xm,Ym2])
                        polygons.insert(2,polyBHD)

                        
                    #                    polygon = Polygon([(Xm,Ym2),(Xm2,Ym),(X4,YS),(X3,YS2),(X3,1.0),(X4,1.0),(XL,1.0),(XL,0.0)],True) #WBF
                        polyWBF.append([Xm,Ym2])
                        polyWBF.append([Xm2,Ym])
                        polyWBF.append([X4,YS])
                        polyWBF.append([X3,YS2])
                        polyWBF.append([X3,1.0])
                        polyWBF.append([X4,1.0])
                        polyWBF.append([XL,1.0])
                        polyWBF.append([XL,0.0])
                        polyWBF.append([Xm,Ym2])
                        polygons.insert(2,polyWBF)

                    elif Ym == 1.0:
                        #                    polygon = Polygon([(Xm,Ym2),(Xm2,Ym),(X4,1.0),(X4,YS),(X2,YF2),(XL,0.0)],True) #BHD
                        polyBHD.append([Xm,Ym2])
                        polyBHD.append([Xm2,Ym])
                        polyBHD.append([X4,1.0])
                        polyBHD.append([X4,YS])
                        polyBHD.append([X2,YF2])
                        polyBHD.append([XL,0.0])
                        polyBHD.append([Xm,Ym2])
                        polygons.insert(2,polyBHD)
                        
                    #                    polygon = Polygon([(X4,1.0),(X3,1.0),(X3,YS2),(X4,YS),(X4,1.0),(Xm2,1.0),(XL,1.0),(XL,0.0),(Xm,Ym2),(Xm2,Ym)],False) #WBF
                        polyWBF.append([X4,1.0])
                        polyWBF.append([X3,1.0])
                        polyWBF.append([X3,YS2])
                        polyWBF.append([X4,YS])
                    ...
                    if Ym == 0.0 and Ym2 == 0.0:
                        ...
                        polyBHD.append([Xm,Ym2])
                        polyBHD.append([X4,Ym])
                        polyBHD.append([Xm,Ym2])
                        polygons.insert(2,polyBHD)
                        
                    #                    polygon = Polygon([(X4,YS),(X3,YS2),(X3,1.0),(X4,1.0),(XL,1.0),(XL,0.0),(X2,0.0)],True) #WBF
                        polyWBF.append([X4,YS])
                        polyWBF.append([X3,YS2])
                        polyWBF.append([X3,1.0])
                        polyWBF.append([X4,1.0])
                    ...
                    #                    polygon = Polygon([(Xm,Ym2),(X4,Ym),(X4,YS),(X3,YS2),(X3,1.0),(X4,1.0),(XL,1.0),(XL,0.0)],True) #WBF
                        polyWBF.append([Xm,Ym2])
                        polyWBF.append([X4,Ym])
                        polyWBF.append([X4,YS])
                        polyWBF.append([X3,YS2])
                        polyWBF.append([X3,1.0])
                        polyWBF.append([X4,1.0])
                ...
                if Xm2 > X4:
                    if Ym <  YS:
                        ...
                        polyBHD.append([Xm,Ym2])
                        polyBHD.append([Xm2,Ym])
                        polyBHD.append([XL,YL])
                        polyBHD.append([Xm,Ym2])
                        polygons.insert(2,polyBHD)
                        
                    #                    polygon = Polygon([(Xm,Ym2),(Xm2,Ym),(X4,YS),(X3,1.0),(X4,1.0),(XL,1.0),(XL,Ym2)],True) #WBF
                        polyWBF.append([Xm,Ym2])
                        polyWBF.append([Xm2,Ym])
                        polyWBF.append([X4,YS])
                        polyWBF.append([X3,1.0])
                    ...
                    #                    polygon = Polygon([(X4,1.0),(X3,1.0),(X4,YS),(X4,1.0),(Xm2,1.0),(XL,1.0),(Xm,Ym2),(Xm2,Ym)],False) #WBF
                        polyWBF.append([X4,1.0])
                        polyWBF.append([X3,1.0])
                        polyWBF.append([X4,YS])
                        polyWBF.append([X4,1.0])
            ...
            if bhdsit == 4:
                if X2 > XL:
                    #                polygon = Polygon([(X4,1.0),(X4,YWIB),(x2,YWIB),(X4,YS),(XL,YL),(XL,1.0)],True) #BHD
                    polyBHD.append([X4,1.0])
                    polyBHD.append([X4,YWIB])
                    polyBHD.append([x2,YWIB])
                    polyBHD.append([X4,YS])
                    polyBHD.append([XL,YL])
                    polyBHD.append([XL,1.0])
                    polyBHD.append([X4,1.0])
                    polygons.insert(2,polyBHD)
                ...
                #                polygon = Polygon([(X4,1.0),(X4,YWIB),(x2,YWIB),(X3,YS2)],True) #WBF
                    polyWBF.append([X4,1.0])
                    polyWBF.append([X4,YWIB])
                    polyWBF.append([x2,YWIB])
                    polyWBF.append([X3,YS2])
                    polyWBF.append([X4,1.0])
                    polygons.insert(2,polyWBF)
                    
                else:
                    #                polygon = Polygon([(X4,1.0),(X4,YWIB),(x2,YWIB),(X4,YS),(X2,YF2),(XL,0.0),(XL,1.0)],True) #BHD
                    polyBHD.append([X4,1.0])
                    polyBHD.append([X4,YWIB])
                    polyBHD.append([x2,YWIB])
                    polyBHD.append([X4,YS])
            ...
            if bhdsit == 5 or bhdsit == 6:
                if YWIB > YS2:
                    #                polygon = Polygon([(X4,1.0),(X4,YWIB),(0.0,YWIB),(0.0,YS2),(X4,YS),(X2,YF2),(XL,YL),(XL,1.0)],True) #BHD
                    polyBHD.append([X4,1.0])
                    polyBHD.append([X4,YWIB])
                    polyBHD.append([0.0,YWIB])
                    polyBHD.append([0.0,YS2])
                    polyBHD.append([X4,YS])
                    polyBHD.append([X2,YF2])
                    polyBHD.append([XL,YL])
                    polyBHD.append([XL,1.0])
                ...
                #                polygon = Polygon([(X4,1.0),(X4,YWIB),(0.0,YWIB),(0.0,1.0)],True) #WBF
                    polyWBF.append([X4,1.0])
                    polyWBF.append([X4,YWIB])
                    polyWBF.append([0.0,YWIB])
                    polyWBF.append([0.0,1.0])
                    polyWBF.append([X4,1.0])
                    polygons.insert(2,polyWBF)
                    
                else:
                    #                polygon = Polygon([(X4,1.0),(X4,YWIB),(x2,YWIB),(X4,YS),(X2,YF2),(XL,YL),(XL,1.0)],True) #BHD
                    polyBHD.append([X4,1.0])
                    polyBHD.append([X4,YWIB])
                    polyBHD.append([x2,YWIB])
                    polyBHD.append([X4,YS])
