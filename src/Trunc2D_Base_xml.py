...
import copy
import numpy as np
from  src.xmlFunctions import getKeyword, getFloatCommand, getTextCommand
...
class Trunc2D_Base:
    ...
    def _setGaussFieldForBackgroundFaciesTruncationMap(self, gaussFieldsInZone, alphaFieldNameForBackGroundFacies,
                                                       nGaussFieldsInBackGroundModel):
        """
        Description: The lists: self._gaussFieldsInZone, self._alphaIndxList and the integer self._nGaussFieldsInTruncationRule
...
#        print(repr(alphaFieldNameForBackGroundFacies))
    ...
    def _setOverlayFaciesDataStructure(self, overlayGroups):
        """
        Description: Initialize data structure from input list of lists.
...
#        print(repr(self._faciesInTruncRule))
        for indx in range(self._nBackGroundFacies, self._nFacies):
            fName = self._faciesInTruncRule[indx]
#            print('Sum prob fraction for {} is {}'
#                  ''.format(fName, sumProbFrac[indx]))
    ...
    def __checkCenterTruncInterval(self):
        for groupIndx in range(self._nGroups):
            ...
            for j in range(nAlpha):
                ...
                assert centerInterval >= 0.0 and centerInterval <= 1.0
    ...
    def __addAlpha(self,alphaName,createErrorIfExist=False):
        """
        Description: Check that alphaName is a valid name of a gauss field for the zone.
...
#        print(repr(self._gaussFieldsInZone))
        ...
        if overLayModelObj is not None:
                ...
                for bgFaciesObj in overLayGroupObj.findall(kw2):
                    ...
                    if indx in checkBGFaciesList:
                        raise ValueError(
                            ...
                            ' The implemented method cannot handle this case.'.format(modelFileName, str(zoneNumber), bgFaciesName)
                            )
                    else:
            ...
            if self._nFacies != (self._nBackGroundFacies + self._nOverLayFacies):
                raise ValueError(
                    ...
                    ''.format(str(zoneNumber), str(self._nFacies), str(self._nOverLayFacies), str(self._nBackGroundFacies))
                )

            if self._debug_level >= Debug.VERY_VERBOSE:
                print('Debug output: List of alpha fields used in the zone model. The sequence define the alpha coordinates')
                for i in range(len(self._alphaIndxList)):
                    indx = self._alphaIndxList[i]
                    alphaName = self._gaussFieldsInZone[indx]
                    print('Debug output:  Alpha coordinate {}  corresponds to gauss field {}'.format(str(i+1), alphaName))
                ...
                for groupIndx in range(self._nGroups):
                    nAlpha = len(self._alphaInGroup[groupIndx])
                    print('Debug output: For group with index: ' + str(groupIndx) + '  Number of alpha fields: ' + str(nAlpha))
                    ...
                    for alphaInGroupIndx in range(nAlpha):
                        ...
                        probFrac = self._probFracOverlayFaciesInGroup[groupIndx][alphaInGroupIndx]
                        print('Debug output:   ' + gfname + '     ' + fName + '      ' + str(centerVal) + '     ' + str(probFrac))
    ...
    def writeContentsInDataStructure(self):
        ...
        print('Logical array with 0 for facies which is has probability 100% and 0 for facies with probability less than 100%')
        ...
        for i in range(self._nGroups):
            ...
            for j in range(nAlpha):
                ...
                print('Overlay facies polygon number {} in group {} belongs to facies {} with prob fraction {} and defined by truncation of {}'
                      ''.format(str(j), str(i), fName, probFraction, gfName))
                print('Overlay facies polygon number {} in group {} belongs to facies {} and is truncated between {} and {}'
    ...
    def setEpsFaciesProb (self,eps):
        if eps > 0.0 and eps < 0.01:
            self._epsFaciesProb = eps
        else:
            raise ValueError('Try to set tolerance for facies probabilities to {}.\n'
...
#TODO: Sjekk ut robusthet av algoritmen i spesialtilfeller med 0 sannsynlighet for en eller flere facies eller hele grupper av facies
    def _modifyBackgroundFaciesArea(self, faciesProb):
        ...
        for groupIndx in range(self._nGroups):
            ...
            if self._debug_level >= Debug.VERY_VERBOSE:
                print('Debug output: Group {}: Background facies prob={}  Overlay facies prob={}   Sum prob= {}'
                      ''.format(str(groupIndx), str(sumProbBackGround[groupIndx]), str(sumProbOverlay[groupIndx]), str(sumProb[groupIndx])))
...
#            print(repr(area))
        ...
        if self._debug_level >= Debug.VERY_VERBOSE:
            ...
            print('Debug output: Finished calculate modified area for background facies and threshold values for overlay facies')
            print(' ')
        return area
    ...
    def _truncateOverlayFacies(self, indx, alphaCoord):
        ...
        Description: Is used to truncate and find overlay facies. This function will be used
                     ...
                     2. Within each group, there can be one or more overlay facies. Each overlay facies is defined by
                        ...
                        NOTE: There is a requrement that no two alpha variables (gauss fields) are equal within a group.
        ...
        if groupIndx >= 0:
            # The background facies is defined by the input variable 'indx' and the facies name is faciesInTruncRule[indx].
            # This background facies belongs to the group with index groupIndx. The overlay facies in this group may overprint
            # the background facies. The sequence of overlay facies is determined by the sequence of alpha fields.
            # So the sequence of alpha fields defined by the alphaInGroup and the corresponding sequence of facies define
            # the internal priority of which overlay facies is truncated or not by the other overlay facies in this group.
            # Note that overlay facies from different groups are independent of each other.
            # This code can be extended to have multiple overlay facies defined by the same alpha field,
            # by calculating threshold values for the alpha value for each facies, but so far only one
            # overlay facies is defined for each alpha field. 
            nAlpha = len(self._alphaInGroup[groupIndx])
            for i in range(nAlpha):
    ...
    def _makeKey(self, faciesProb, keyResolution):
        keyList = []
        key = None
        for p in faciesProb:
    ...
    def _makeRoundOfFaciesProb(self, faciesProb, keyResolution):
        faciesProbNew = []
        for i in range(len(faciesProb)):
            p = faciesProb[i]
