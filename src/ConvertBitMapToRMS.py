...
#                        interval of pixels corresponding to the rectangle defined by the cornerpoints. 
...
class ConvertBitMapToRMS:
    def __init__(self,modelFileName,defineFaciesCode=0):
        self.__nx = 0
        self.__ny = 0
        self.__iStart = 0
        self.__iEnd = 0
        ...
        self.__crop = 0
        self.__xminUnCropped = 0
        self.__xmaxUnCropped = 0
        self.__yminUnCropped = 0
        self.__ymaxUnCropped = 0
        ...
        obj = root.find(kw)
        if obj is None:
            print('Error reading model file {}. Missing command: {}'.format(modelFileName, kw))
            raise IOError()
        kw = 'xmin'
        obj2 = obj.find(kw)
        if obj2 is None:
        ...
        if self.__iStart < 1:
            print('Error: Pixel interval in y direction ' + '(' + str(self.__iStart) + ',' + str(self.__iEnd) + ')' 
                  + ' is not within ' + str(1) + ' and ' + str(self.__ny))
            raise ValueError()

        if self.__jStart < 1:
            print('Error: Pixel interval in x direction ' + '(' + str(self.__jStart) + ',' + str(self.__jEnd) + ')' 
        ...
        if obj is not None:
            text = obj.text
            self.__crop = int(text.strip())

        if defineFaciesCode == 1:
            kw = 'MissingCode'
            obj = root.find(kw)
            if obj is None:
                print('Error reading model file {}. Missing command {}\n'.format(modelFileName, kw))
                raise IOError()
...
#            pixelCounts = []
#            pixelCounts = im.histogram()
            # print( repr(pixelCounts))
    ...
    #        if isNumber(c):
            if isOneByteColor(c):
                ...
                if isThreeByteColor(c):
                    print('Error: Number of bytes per pixel is not 1 but 3\n'
            ...
            if nrows != nrowSpecified or ncols != ncolSpecified:
                print('Error: Number of pixel specified in grid definition is different from the number of pixels in the map\n'
                      '       Number of pixels in bitmap file: (nx,ny) = ' + str(ncols) + ' ' + str(nrows)+'\n'
                      '       Number of pixels specified:      (nx,ny) = ' + str(ncolSpecified) + ' ' + str(nrowSpecified))
                raise ValueError
            ...
            if self.__nFacies > 0:
                missingCode = self.__missingCode
    ...
    def __minMaxCoordForUnCroppedMaps(self):
        self.__xminUnCropped = self.__xOrigo - self.__xinc*(self.__jStart-1)
        self.__xmaxUnCropped = self.__xMax   + self.__xinc*(self.__nx - self.__jEnd)
        self.__yminUnCropped = self.__yOrigo - self.__yinc*(self.__iStart-1)
        self.__ymaxUnCropped = self.__yMax   + self.__yinc*(self.__ny - self.__iEnd)
    ...
    def writeFile(self):
        if self.__crop:
            ...
            self.__minMaxCoordForUnCroppedMaps()
            xmin = self.__xminUnCropped
            ymin = self.__yminUnCropped
