...
class Trunc2D_Angle_Overlay:
    ...
    def __init__(self,trRuleXML=None, mainFaciesTable=None, faciesInZone=None, 
        ...
        err = self.__interpretXMLTree(trRuleXML,mainFaciesTable, faciesInZone,printInfo,modelFileName)
        if err == 1:
            sys.exit()
        return
    ...
    def __interpretXMLTree(self,trRuleXML,mainFaciesTable, faciesInZone,printInfo,modelFileName):
        ...
        for faciesObj in trRuleXML.findall(kw):
            if faciesObj == None:
                ...
                print('Error: Missing keyword ' + kw + ' under keyword ' + ' TruncationRule')
                sys.exit()
            ...
            if angleObj == None:
                print('Error when reading model file: ' + modelFileName)
                print('Error: Read truncation rule: ' + self.__className)
                print('Error: Missing keyword ' + kw2 + ' under keyword Facies under keyword TruncationRule')
                sys.exit()
            text = angleObj.text
            ...
            if pFracObj == None:
                print('Error when reading model file: ' + modelFileName)
                print('Error: Read truncation rule: ' + self.__className)
                print('Error: Missing keyword ' + kw3 + ' under keyword Facies under keyword TruncationRule')
                sys.exit()
            probFrac = float(pFracObj.text)
            if probFrac < 0.0 or probFrac > 1.0:
                print('Error when reading model file: ' + modelFileName)
                print('Error: Read truncation rule: ' + self.__className)
                text = 'Error: Probability fraction specified for ' + fName + ' in keyword ' + kw3 + '\n'
                text = text + ' under keyword Facies under keyword TruncationRule' 
                text = text + ' is: ' + str(probFrac) + ' which is outside [0,1]'
                print(text)
                sys.exit()
    ...
    def __addFaciesToTruncRule(self,fName):
        ...
        if fIndx == -999:
            print('Error in ' + self.__className)
            print('Error: Specified facies name in truncation rule is not defined for the zone')
            sys.exit()
    ...
    def initialize(self,mainFaciesTable,faciesInZone,truncStructure,
        ...
        err=0
        self.__printInfo = printInfo
        self.__mainFaciesTable = copy.copy(mainFaciesTable) # Main facies table
        self.__nFaciesMain = self.__mainFaciesTable.getNFacies()

        self.__faciesInZone = copy.copy(faciesInZone)  # Facies with defined probabilities for the zone 
        self.__nFacies = len(self.__faciesInZone)
        self.__faciesIsDetermined = np.zeros(self.__nFacies,int)
        ...
        if len(backgroundFacies) > 0:
            ...
            if isNewFacies == 1:
                ...
                print('Error in ' + self.__className)
                text = 'Error: Wrong name of overlay facies. '
                text = text + 'This facies name is already used in truncation rule as a possible background facies.'
                print(text)
                err = 1
        # End if use overlay
...
#        for i in range(self.__nFacies):
    ...
    def __findSminSmaxForPolygon(self,polygon,vx,vy,vxNormal,vyNormal,x0Normal,y0Normal):
        ...
        for i in range(n):
            ...
            s = (-vy*(x0-x0Normal) + vx*(y0-y0Normal))/d  # This number must be value such that     0 <= s <= 1
            if s < smin:
                smin = s
            if s > smax:
                smax = s
    ...
    #    print( ' smin,smax: ' + str(smin) + ' '  + str(smax))
        ...
        for i in range(nmax):
            ...
            if isSplit:
                ...
                print('Error in ' + self.__className)
                print('Error: When calculating a split of the polygon.')
                sys.exit()
            ...
            for i in range(self.__nPolygons):
                ...
                if inside == 0:
                    ...
                    if self.__isBackgroundFacies[indx] == 1:
                        if z> self.__lowH and z < self.__highH:
...
#            print('Use overlay facies: No')
    ...
    def XMLAddElement(self,parent):
        ...
        attribute = {'name':'Trunc2D_Angle_Overlay','nGFields':'3'}
        tag        = 'TruncationRule'
