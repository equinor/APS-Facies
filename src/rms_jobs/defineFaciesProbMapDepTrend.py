...
# -*- coding: utf-8 -*-
'''
Description: The script calculates a one dimensional trend of volume fraction of each cathegory (facies) in an input RMS discrete 3D parameter.
             The output is a 3D RMS parameter of type continuous containing the volume fraction of each cathegory (facies) input RMS discrete 3D parameter.
             The user specify a direction, an azimut angle and in this direction a set of stripes orthogonal to the specified direction is defined. 
             ...
             cathegory or facies is calculated. This creates a 1 dimensional trend varying in the azimuth direction but not orthogonal to the azimuth direction. 
             This type of trend can be used to as facies probabilties  in cases one want to use trends in probability instead of in Gaussian fields in 
             the APS method. The trend created here is a bit experimental and is probabily only relevant if the depositional system of facies has a clear 
...
Example of model file:
             ...
             the same setting is defined to be used for all these zones when calculating the facies (cathegorical variable) probability trend.
