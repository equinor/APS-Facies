...
# -*- coding: utf-8 -*-
''' 
Description: The script defineFaciesProbTrend can be used to make RMS 3D parameters for facies probabilities to be used in APS facies modelling.
             The output is facies probabilities or in special case volume fraction of each cathegory defined in input RMS 3D discrete parameter.
             A model file (xml file) is specified by the user and define how the output probability cubes (RMS3D continuous parameters) is calculated.
             ...
             the input RMS 3D parameter. For example, if the input contains 3 different values corresponding to the three values for the code names:
                Name  Code
                 A     1
                 B     2
                 C     3
             and the user want to model the following facies:
                Name   Code
                 ...
                 F3    3
                 F4    4
             then the user must specify conditioned probabilities for each facies to be modelled given the cathegory value from input like e.g.
                Prob(F1|A)  = 0.9
                Prob(F2|A)  = 0.05
                Prob(F3|A)  = 0.05
                Prob(F4|A)  = 0.0
             ...
             input RMS 3D parameter has cathegory A, 0.1 in grid cells where input is B, 0.0 in grid cells where 
             input is C.
             For facies F2, F3, F4 the procedure for assigning probabilities is the same.
             Note for consistency, the sum of probabilities for F1, F2 F3 and F4 must sum to 1.0 in each grid cell. This means that e.g.
             Prob(F1|A) + Prob(F2|A) + Prob(F3|A) + Prob(F4|A) = 1 and similar for  grid cell in input where the cathegory is B,C.

             Some special cases of specification:
             - One to one correspondence between facies to be modelled and the input cathegorical variables:
               Assume the facies numbers of modelled facies is 3, the same as the number of cathegories of input discrete RMS parameter.
             ...
             A special case of usage of the average probability option is just to set Prob(F1|A) = 1 Prob(F2|B) = 1 Prob(F3|C) = 1. In this case
             the average probabilities calculated is the same as the fraction of grid cells of cathegory A relative to all cells in the zone, 
             and so on for B and C also. This is just the volume fraction of A, B and C in each zone and 
             the output of facies probability F1 is actually the volume fraction of A. probability of F2 is the volume fraction of B and so on.
...
Example of a model file:
            ...
            is the facies name to be modelled, and the second name is the name of the cathegory in in input discrete facies parameter. 
            The third entry is the probability. The command UseConstantProbFromVolumeFraction is set to 1 if the user 
            wants the average probability as output in each cell instead of the specified probability. The command SelectedZones define 
            zone number for the zone this model is  defined for. If more than one zone can use the same specification, 
            multiple zone numbers can be specified with a space in between in this command. 
'''
