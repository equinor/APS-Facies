...
def simGaussField( iseed: int, nx: int, ny: int, xsize: float, ysize: float, variogram_type: VariogramType,
    ...
    # Define variogram
    variogramName = variogram_type.name.upper()
    if variogramName == 'GENERAL_EXPONENTIAL':
        simVariogram = nrlib.variogram(variogramName,  main_range=range_major_axis,  perp_range=range_minor_axis, 
                                       azimuth=azimuth_angle, power=power)
    else:
        simVariogram = nrlib.variogram(variogramName,  main_range=range_major_axis,  perp_range=range_minor_axis, 
    ...
    tmpVector = nrlib.simulate(simVariogram, nx, dx, ny, dy)
    a = np.reshape(tmpVector, (nx,ny),'F')
    gaussVector = np.reshape(a,(nx*ny),'F')
    return gaussVector
