...
def simulateGauss(simParamObject,outputDir,logFileName=None, seedFileName=None, writeSeedFile=True, debug_level=Debug.OFF):
    import nrlib
                    ...
                    for i in range(len(words)):
                        ...
                        if w == gaussFieldName:
                            ...
                            if zNr == zoneNumber and rNr == regionNumber:
                                ...
                                if debug_level >= Debug.VERY_VERBOSE:
                                    print('  Read seed: {} from seed file: {}'
                                          ''.format(str(startSeed), seedFileName)
                                          )
                                break
                    if startSeed == -1:
                        raise IOError('The seed file: {} does not contain seed value for gauss field name: {}   zone number: {}    and region number: {}'
                                      ''.format(seedFileName, str(zoneNumber), str(regionNumber))
                                      )
            except:
...
def run_simulations(
    ...
    for key, zoneModel in allZoneModels.items():
        ...
        for i in range(len(gaussFieldNames)):
            ...
            simGaussProcess = mp.Process(target=simulateGauss,args=(simParam, outputDir, 
                                                                    logFileName, 
                                                                    seedFileNameForThisProcess, 
                                                                    writeSeedFile, debug_level))
