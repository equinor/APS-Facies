...
Description: This script can be used to create binary (0/1) logs or probability logs from facies logs for blocked wells.
             ...
             - For each facies that is defined as modelling facies for a zone, the probability log is updated. Initially all blocked well
               cells are set to undefined. If the facies from the facies log is one of the modelling facies for the zone, assign
               probability value 1 to the probability log for that facies and assign 0 to probability logs for the other modelling
               facies for the zone. If the facies is not one of the modelling facies for the zone, the probability log value is set to undefined.
               It is important to note that each zone may have different sets of modelling facies.
               If the user wants to model facies that is not observed and hence not found in the blocked well log, the facies can be
               specified to be a modelling facies. The probability log for the unobserved facies will get probability value 0 in
               all blocked well cells where other modelling facies is observed and undefined elsewhere. It is important to also
               include these unobserved facies in the list of modelling facies for the zone to ensure that these facies get 0
               probability in blocked well grid cells.

             - The script has an option to use conditional probabilities which means that the probability P(modelling_facies | observed_facies)
               for the modelling facies in a blocked well grid cell given the observed facies is not only 1 if observed and 0 if not observed
               and undefined if not modelling facies. This option is experimental and must be used with care, but can be a simple alternative
               if the interpretation is uncertain. Note however that each blocked well grid cell is treated independent from other
               blocked well grid cells and no spatial information about correlations are implemented in this simple model.
               The are also some practical restrictions since one in principle will need a full matrix of conditional probabilities
               for each zone. This is not implemented here so either this option can only be used for one zone at a time or all specified
               zones must have identical set of modelling facies.

Input:         Model file in xml format
              ...
              the probability log value is undefined. The normalization of the probabilities take care of this and ensure that sum of
              probabilities is 1 for the facies chosen to be modelled. When running in standard mode where binary logs are calculated,
              the logs defined as modelling facies for a zone have values 1 or 0 and sum to 1.
              ...
              of conditional probabilities. It might work also in the special case that modelled facies are the same for all specified zones.
              and in this case the same conditional probabilities are used for all zones. But it will fail if there are zones with
              different numbers of facies som that modelled facies set is not identical for all specified zones.

Output:      New logs are created in the blocked well set with defined values for the probabilities for the grid cells containing
             facies to be modelled for each individual zone and undefined for other facies. All grid cells in the blocked well set
             where the zone is undefined or the input facies log is undefined will also get undefined for the probabilities. The output
             probability logs will get the name  <prefix>_<modelling_facies_name>.
'''
...
if __name__ == '__main__':
    ...
    run(roxar, project, probability_log_specification_file='Create_prob_logs.xml', debug_level=Debug.VERBOSE)
