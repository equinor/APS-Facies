...
truncation rules.The specified overlay facies is not shown in the truncation map in previewer since the truncation rule will be a $2+N$ dimensional cube where $N$ is number of GRF's in addition to the two first that is used to define the background facies.
...
(Center is a number between 0 and 1 with default 0).If multiple polygons are specified for an overlay facies,
...
The interval along the Alpah3 axis is divided into $[0,s]$ for overlay facies and $[s,1]$ where background facies is used, and s is a calculated threshold value that depends on the facies probability.
...
The calculation of the threshold values along the Alpha3, Alpha4, ... AlphaK axes are calculated such that the $2 + N$ dimensional truncation cube is split consistently into $2 + N$ dimensional polygons with volume fractions that match the facies probabilities for each facies.
