...
- When testing a setup for a new APS model, it is useful to specify constant probabilities (not toggle on "Use cubes").

APS models with probability cubes:

- A probability cube is a 3D continuous RMS parameter with a probability in each grid cell for a facies to be present.
  APS will need one 3D parameter for each facies to be modelled containing the facies probability.
