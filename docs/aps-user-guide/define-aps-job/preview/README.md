...
move the mouse icon over the icon and a message will tell you that the specified model is incomplete or has errors.
Fix the missing specification of the model for the current zone and try again.

Note that when using GRF trends of type `RMS_PARAM` or `RMS_TRENDMAP`, the preview will be unavailable since no preview is implemented for these two trend types for GRF.
