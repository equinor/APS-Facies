...
if TYPE_CHECKING:
    ...
    from aps.utils.roxar.job import JobConfig
...
def dump_debug_information(config: 'JobConfig') -> None:
    ...
    when = time.localtime()
    prefix = (
        '{year}{month:02}{day:02}-{hour:02}{minute:02}{second:02}-{random}-APS'.format(
            year=when.tm_year,
            month=when.tm_mon,
            day=when.tm_mday,
            hour=when.tm_hour,
    ...
    with ZipFile(prefix + '.zip', mode='x', compression=ZIP_DEFLATED) as zipfile:
        ...
        def dump(
            file_name: str, data: Union[str, bytes], encoding: str = 'UTF-8'
        ) -> None:
