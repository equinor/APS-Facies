...
def check_grid_layout(
    ...
    zone_conformity_dict, boundary_is_sampled = get_conformity_per_zone(
    ...
    if conformity == Conform.Undefined:
        if debug_level >= Debug.ON:
            ...
            print(
                '        The implemented zone conformities is defined by using option'
                " 'Horizon' and not 'Surface'."
            )
            print(
                '        You must choose the best of the three possible implemented conformities:'
            )
    ...
    if grid_layout != conformity:
        if not return_grid_layout:
            ...
            raise ValueError(
                '\n'
                f"Specified zone conformity for zone '{zone_name}' with zone number {zone_number} "
                f"is '{grid_layout}'.\n"
                f"Grid model '{grid_model_name}' has different zone conformity: {conformity}.\n"
                f'Change conformity setting {aps_job_string} to match the correct one used in the grid model.'
            )
        else:
            return conformity
