...
def trend_map_to_grid_param(
    ...
    grid_points = _snap_points_to_surface(
        project, grid_points, trend_map_name, zone_name
    )
    ...
    _create_property_and_store(
        project, grid_model_name, grid, grid_points, kminmax, result_param_name
    )
...
def _create_property_and_store(
    project, grid_model_name, grid, gridpoints, kminmax, result_param_name
):
    """Create the property by some clever numpy mapping."""

    ixn = (
        gridpoints.dataframe['IX'].values.astype('int') - 1
    )  # -1 since df has 1 based index
    jyn = gridpoints.dataframe['JY'].values.astype('int') - 1
    zvalues = gridpoints.dataframe['VAL'].values

    try:
        newprop = xtgeo.gridproperty_from_roxar(
            project,
            grid_model_name,
            result_param_name,
            realisation=project.current_realisation,
        ...
        if 'No property in' in str(
            verror
        ) or 'The requested grid property has no data.' in str(verror):
            newprop = xtgeo.GridProperty(grid)
        else:
            raise
    krange = np.linspace(
        kminmax[0], kminmax[1], kminmax[1] - kminmax[0] + 1, dtype=np.int32
    )
