...
def init_rms_param(grid_model, name, param_type_name,
        number_of_active_cells, realization_number,
        set_rms_param=False, is_shared=False, debug_level=Debug.OFF):
    ...
    try:
        ...
        values = getContinuous3DParameterValues(grid_model, parameter_name, realization_number)
    except ValueError:
        ...
        if set_rms_param:
            set_continuous_3d_parameter_values(
                grid_model, parameter_name, values, [],
...
def initialize_rms_parameters(
    ...
    gauss_field_names_used, gauss_field_names_with_trend = get_all_gauss_field_names_in_model(aps_model)
    ...
    for name in gauss_field_names_used:
        if active_output_variable_list[0]:
            # Simulated GRF
            gf_all_values[name] = init_rms_param(grid_model, name, None,
                number_of_active_cells, realization_number, True, is_shared,
                debug_level=debug_level)
        if active_output_variable_list [1]:
            # Transformed GRF
            gf_all_alpha[name] = init_rms_param(grid_model, name, '_transf',
                number_of_active_cells, realization_number, set_rms_param, is_shared,
                debug_level=debug_level)

    for name in gauss_field_names_with_trend:
        if active_output_variable_list[2]:
            # Trend for GRF
            gf_all_trend_values[name] = init_rms_param(grid_model, name, '_trend',
                number_of_active_cells, realization_number, set_rms_param, is_shared,
                debug_level=debug_level)
        if fmu_with_residual_grf and active_output_variable_list[3]:
                # Residual GRF
                gf_all_residual_values[name] = init_rms_param(grid_model, name, '_residual',
                    number_of_active_cells, realization_number, set_rms_param, is_shared,
                    debug_level=debug_level)
