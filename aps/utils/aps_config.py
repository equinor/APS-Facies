...
class APSConfig:
    ...
    def init(
        ...
        cls.project = project
        cls.rms_project_directory_path = str(
            Path(cls.project.filename).parent.absolute()
        )
        cls.config_file = (
            cls.rms_project_directory_path + '/' + cls.config_file_without_path
        )
    ...
    def project_dir(cls):
        if cls.project is None:
            raise ValueError(
                'Internal error: Must initialize APSConfig with init function'
            )
    ...
    def global_variables_file(cls):
        return (
            cls.top_dir() + '/' + cls.config['relative_paths']['global_variables_file']
        )
    ...
    def _read_aps_config(cls) -> dict:
        ...
        if len(missing_msg) > 0:
            print(
                'Error: The following keys or subkeys are not specified '
        ...
        if len(undefined_msg) > 0:
            print(
                f'Warning: The following keys are not defined and will be '
    ...
    def _create_aps_config_file(cls):
        ...
        print(' ')
        print(
            f'NOTE: File with configuration of standard FMU directories: {cls.config_file} will be created.'
        )
        print(
            '      If your FMU project use non-standard FMU directory structure or non-standard file name'
        )
        print('      for global variables, please open and edit this file.')
        print(
            f'      Close and open again the APS job to be sure your modified configuration file: {cls.config_file} is used.'
        )
        print(' ')
        with open(cls.config_file, 'w', encoding='utf-8') as file:
            ...
            file.write(
                '# When using standard FMU directory structure and file names,\n'
            )
            file.write(
                '# turn off using non-standard FMU settings in APS settings and this file will not be used.\n'
            )
            file.write(
                '# For FMU project using non-standard directory structure or alternative name for file with\n'
            )
            file.write(
                '# global variables, modify the settings here, but keep all keywords.\n'
            )
