...
class ReadingXmlError(ApsXmlError, IOError):
    def __init__(self, keyword, parent_keyword=None, model_file_name=None):
        message = self.get_keyword_error_message(
            keyword, parent_keyword, model_file_name
        )
...
class ValueOutsideExpectedRange(ApsXmlError):
    ...
    @staticmethod
    def get_value_specified_error_message(
        keyword, parent_keyword=None, model_file_name=None, end_with_period=True
    ):
...
class LessThanExpected(ValueOutsideExpectedRange):
    def __init__(
        ...
        message = (
            base_message + f', is LESS than the minimum value ({value} < {minimum})'
        )
        if end_with_period:
            message += '.'
...
class MoreThanExpected(ValueOutsideExpectedRange):
    def __init__(
        ...
        message = (
            base_message + f', is GREATER than the maximum value ({value} > {maximum})'
        )
        if end_with_period:
            message += '.'
        super().__init__(message)
