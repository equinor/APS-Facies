...
def check_probability_values(
    ...
    if negative_fraction > max_allowed_fraction_with_mismatch:
        ...
        err_list.append(f'Number of grid cells with probability values < {-tolerance_of_probability_normalisation}:  {num_negative}')
        err_list.append(f'Fraction of grid cells with probability values  < {-tolerance_of_probability_normalisation}:  {negative_fraction * 100:.2f}%')
        if stop_on_error:
...
def check_probability_normalisation(
    ...
    if not np.allclose(sum_probability_values, ones, eps):
        ...
        if unacceptable_prob_normalisation_fraction > max_allowed_fraction_with_mismatch:
            ...
            err_list.append(f'Tolerance interval is: [{low_tolerance}, {high_tolerance}]')
            err_list.append(f'Number of grid cells with probabilities summing up to value outside tolerance interval: {unacceptable_prob_normalisation}')
            err_list.append(f'Fraction of grid cells outside tolerance: {unacceptable_prob_normalisation_fraction * 100:.2f}%')
            err_list.append(f'Max limit of mismatch fraction specified: {max_allowed_fraction_with_mismatch * 100:.2f}%')
            ...
            err_list.append('If you think the probability cubes are OK anyway, you may increase the maximum fraction in project settings to ')
            err_list.append(f'at least {unacceptable_prob_normalisation_fraction * 100:.2f}%.')
            if stop_on_error:
                for item in err_list:
                    print(item)
