    ...
    **kwargs,
    ...
    def _get_grid_ijk_handedness(grid_model_name):
        ...
        if rms_grid_model.is_empty(project.current_realisation):
            raise ValueError(
                f'Specified grid model: {grid_model_name} is empty for realisation {project.current_realisation}.'
            )
    ...
    if fmu_mode or fmu_mode_only_param:
        if not is_grid_model_shared(rms_grid_name):
            try:
                ...
                roxar.rms.error(
                    'APS should use shared grid models when running in FMU mode'
                )
            except AttributeError:
                pass
            raise ValueError(
        ...
        if not fmu_mode_only_param:
            ...
            if fmu_ijk_handedness != rms_ijk_handedness:
                try:
                    ...
                    # Introduced in version 12.1.
                    roxar.rms.error(
                        'APS workflows require same grid index origin for both modelling grid and ERTBOX grid'
                    )
                except AttributeError:
                    pass
                raise ValueError(
