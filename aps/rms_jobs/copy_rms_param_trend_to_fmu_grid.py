...
def copy_from_geo_to_ertbox_grid(
    ...
    for zone_name, zone_item in zone_dict.items():
        ...
        ertbox_active = active_params_in_zone(
            ...
            nx, ny, nz, nz_ertbox,
            conformity,
            number_layers,start_layer, end_layer,
        )
...
def  active_params_in_zone(
        geo_i_indices, geo_j_indices, geo_k_indices,
        geo_nx, geo_ny, geo_nz, nz_ertbox,
        conformity,
        number_layers,start_layer, end_layer):
    ...
    if conformity in [Conform.BaseConform]:
        # Only copy the geogrid layers for the zone into the lowermost ertbox layers
        ertbox_active_3d[:,:,-number_layers:] = active_3d[:,:,start_layer:(end_layer+1)]

    elif conformity in[Conform.TopConform, Conform.Proportional]:
        # Only copy the geogrid layers for the zone into the uppermost ertbox layers
        ertbox_active_3d[:,:,0:number_layers] = active_3d[:,:,start_layer:(end_layer+1)]
    else:
        raise NotImplementedError(
            f"Grid conformity: {conformity} is not supported."
        )
...
def ertbox_active_param_to_rms(prefix, real_number, zone_name, ertbox_grid_model, ertbox_active, debug_level):
