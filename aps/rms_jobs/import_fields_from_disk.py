...
def run(project, model_file, geo_grid_name, load_dir=None, **kwargs):
    ...
    if import_from_ert:
        print(
            f'Import parameters for ERT updated ensemble from files into {fmu_grid_name} and {geo_grid_name}'
        )
    else:
        print(
            f'Import parameters for initial ensemble from files into {fmu_grid_name} and {geo_grid_name}'
        )
        ...
        if aps_model.isSelected(zone.zone_number, zone.region_number):
            ...
            for full_field_name in zone.gaussian_fields_in_truncation_rule:
                ...
                if field_location.exists():
                    ...
                    print('ERROR:')
                    print(
                        f'The file {field_location} for the parameter {full_field_name} is not found.'
                    )
                    print('Check that ERT has created the file if ERT iteration > 0')
                    print(
                        'NOTE: APS requires that all GRF files generated by APS jobs are available in RUNPATH'
                    )
                    print('top level after an update by ERT (iteration > 0).')
                    print(
                        'If you intend not to update all GRF files used by APS jobs in your workflow,'
                    )
                    print('you must create symlinks on top level for RUNPATH to the ')
                    print('initial ensemble version of the GRF field parameter files.')
                    print('The initial ensemble version of the GRF files are usually')
                    print('located under rms/output/aps in RUNPATH under iteration 0.')
                    print(
                        'There exist a script (to be used as a FORWARD model in ERT) to setup the symlinks.'
                    )
