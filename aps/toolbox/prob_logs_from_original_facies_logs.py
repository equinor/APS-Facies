...
def run(params):
    ...
    facies_code_names_dict, zone_code_names_dict = get_facies_and_zone_logs(
        project,
        well_name_list,
        trajectory,
        logrun,
    ...
    if debug_level >= Debug.ON:
        print('- Facies and zones used when calculating probability logs:')
    ...
    if prob_cond_input_dict is not None:
        prob_cond_dict = get_cond_prob_dict(
            prob_cond_input_dict, zone_number_list, common_facies_list=False
        )
    if debug_level >= Debug.VERBOSE:
        if prob_cond_dict is not None:
            sorted_by_zone_dict = dict(
                sorted(
                    prob_cond_dict.items(),
                    key=lambda item: (item[0][0], item[0][2], item[0][1]),
                )
            )
            print(
                '-- Specified conditional probabilities for modelled facies given interpretd facies:'
            )
    ...
    prob_log_names = prob_logs_per_facies(
        project,
        well_name_list,
    ...
    print(f'Blocked well jobs: {blocked_well_job_list=}')
    for jobname in blocked_well_job_list:
        if debug_level >= Debug.ON:
            print(' ')
            print(
                f'- Calculate probability logs for blocked wells defined by the job: {jobname}'
            )

        bw_name = blockwells_prob_logs(
            project,
            grid_model_name,
    ...
    for well_name in well_name_list:
        ...
        for code in facies_code_names_dict.keys():
            if code not in facies_code_mapping.keys():
                ...
                err_msg_list.append(err_msg)
                err_msg = 'Edit facies code mapping.'
                err_msg_list.append(err_msg)
        if len(err_msg_list) > 0:
            ...
            for err_msg in err_msg_list:
                print(err_msg)
            raise ValueError(
                f'Some facies codes found in the log {facies_log_name} is not defined in the facies code mapping.'
            )
        ...
        if not prob_cond:
                ...
                for code in facies_code_names_dict.keys():
                    ...
                    if facies_name in modelling_facies_per_zone_dict[zone_number]:
                        ...
                        if facies_name not in facies_names_used:
                            ...
                            selected = (df_well[facies_log_name] != code) & (
                                df_well[zone_log_name] == zone_number
                            )
                            ...
                            df_well.loc[
                                np.isnan(df_well[facies_log_name]), prob_log_name
                            ] = np.nan
                        else:
                ...
                for cond_code in facies_code_names_dict.keys():
                    ...
                    for code in facies_code_names_dict.keys():
                        ...
                        if facies_name in modelling_facies_per_zone_dict[zone_number]:
                            if (
                                cond_facies
                                in modelling_facies_per_zone_dict[zone_number]
                            ):
                                prob_log_name = prefix_prob_logs + '_' + facies_name
                                ...
                                if prob < 0 or prob > 1.0:
                                    ...
                                    selected = (
                                        df_well[facies_log_name] == cond_code
                                    ) & (df_well[zone_log_name] == zone_number)
                                    df_well.loc[selected, prob_log_name] = prob
                                    df_well.loc[
                                        np.isnan(df_well[facies_log_name]),
                                        prob_log_name,
                                    ] = np.nan
            ...
            if len(err_msg_list) > 0:
                for err_msg in err_msg_list:
                    print(err_msg)
                raise ValueError(
                    'Errors in specification of conditional probabilities for modelled facies. Check specification.'
                )
        ...
        well.to_roxar(
            project, well_name, lognames='all', trajectory=trajectory, logrun=logrun
        )

    return prob_log_names


def blockwells_prob_logs(
    project,
    grid_model_name: str,
    ...
    bw_job = roxar.jobs.Job.get_job(
        owner=['Grid models', grid_model_name, 'Grid'],
        type='Block Wells',
    ...
    if debug_level >= Debug.VERY_VERBOSE:
        print(' ')
        print('--- Initial values of blocked well job parameter setting:')
        pp.pprint(params)
    ...
    if update:
        ...
        if debug_level >= Debug.VERY_VERBOSE:
            print(' ')
            print('--- Updated blocked well job parameter settings:')
            pp.pprint(params)
        ...
        if check_params:
            ...
            if len(err_msg_list) > 0:
                ...
                for err_msg in err_msg_list:
                    print(err_msg)
                raise ValueError(
                    f'Some errors found when updating blocked well job parameters for  {bw_job.name}  of type {bw_job.type} '
                )
    ...
    if debug_level >= Debug.ON:
        print(
            f'- Run well blocking job: {blocked_well_job_name} and update: {blocked_wells_name}.'
        )
    bw_job.execute()
...
def define_new_bw_param(
    bw_log_name: str, log_run_name: str, debug_level: Debug = Debug.OFF
):
    bw_log = {}
...
def normalize_and_check(
    ...
    if debug_level >= Debug.ON:
        print(
            f'- Normalize probability logs in blocked wells set {blocked_wells_name} '
        )
    blocked_wells = project.grid_models[grid_model_name].blocked_wells_set[
        blocked_wells_name
    ]
    ...
    if not bias_specified:
        ...
        if debug_level >= Debug.ON:
            print(
                'Use weight factors to create biased blocked well facies probabilities'
            )
            pp = pprint.PrettyPrinter()
    ...
    for name in blocked_wells.get_well_names():
        if debug_level >= Debug.VERBOSE:
            print(
                f'-- Check and normalize prob logs in blocked well set {blocked_wells_name} and well {name}'
            )
        lognames = blocked_well_prob_log_names.copy()
        lognames.append(blocked_well_zone_log_name)
        bw_well = xtgeo.blockedwell_from_roxar(
            project, grid_model_name, blocked_wells_name, name, lognames=lognames
        )
        ...
        for zone_number, facies_list in modelling_facies_per_zone_dict.items():
            ...
            for fname in facies_list:
                ...
                prob_log = df_blocked_well.loc[selected, prob_log_name] * float(
                    bias_weighting[fname]
                )
                sum_prob_log = sum_prob_log + prob_log

            if not bias_specified:
                # Check if the normalization is far away from 1
                checked_cell_values = sum_prob_log[
                    (sum_prob_log > (1 + eps)) | (sum_prob_log < (1 - eps))
                ]
                n_cells_without_normalization = len(checked_cell_values)
                if n_cells_without_normalization > 0:
                    print(
                        f'Warning: Number of blocked well grid cells from {blocked_wells_name} for well {name} with sum '
        ...
        for zone_number, facies_list in modelling_facies_per_zone_dict.items():
            ...
            for fname in facies_list:
                ...
                sum_prob_log = sum_prob_log + prob_log
            checked_cell_values = sum_prob_log[
                (sum_prob_log > 1.001) | (sum_prob_log < 0.999)
            ]
            n_cells_without_normalization = len(checked_cell_values)
            if n_cells_without_normalization > 0:
                print(
                    f'Error: Number of blocked well grid cells not normalized: {n_cells_without_normalization}  '
                )
        ...
        bw_well.to_roxar(
            project, grid_model_name, blocked_wells_name, name, lognames='all'
        )


def average_prob_logs(
    project,
    grid_model_name: str,
    bw_name: str,
    ...
    number_of_layers_per_zone, start_layers_per_zone, end_layers_per_zone = (
        get_zone_layer_numbering(grid3D)
    )
    ...
    for well_name in well_list:
        bw_well = xtgeo.blockedwell_from_roxar(
            project, grid_model_name, bw_name, well_name, lognames='all'
        )
            ...
            with open(trend_file_name, 'w') as file:
                if sim_box_thickness is not None:
                    ...
                    for k in range(start, end + 1):
                        z = (k - start) * dz
                        file.write(
                            f'{z:.2f} {average_prob_zone_layer[znr, log_number, k]:.2f}\n'
                        )
                    z = (end - start + 1) * dz
                    file.write(
                        f'{z:.2f} {average_prob_zone_layer[znr, log_number, end]:.2f}\n'
                    )
                else:
                    # nlayers, one point per layer
                    for k in range(start, end + 1):
                        file.write(
                            f'{k - start} {average_prob_zone_layer[znr, log_number, k]:.2f} 0\n'
                        )
...
def get_facies_and_zone_logs(
    ...
    for well_name in well_name_list:
        ...
        if not current_facies_code_names_dict:
            raise ValueError(
                f'Facies log {facies_log_name} is not found in well {well_name}'
            )
        if not current_zone_code_names_dict:
            raise ValueError(
                f'Zone log {zone_log_name} is not found in well {well_name}'
            )
...
def _read_model_file_yml(model_file_name):
    ...
    modelling_facies_per_zone_input_dict = get_dict(
        spec, parent_kw, 'ModellingFaciesPerZone'
    )
    debug_level = get_int_value(
        spec, parent_kw, 'DebugLevel', has_default=True, default_value=0
    )
    sim_box_thickness_per_zone_dict = get_dict(
        spec, parent_kw, 'SimBoxThicknessPerZone'
    )
    ...
    facies_mapping_dict = get_dict(
        orig_spec_dict, kw_orig, 'MergeFacies', accept_none=True
    )
    prob_cond_dict = get_dict(
        orig_spec_dict, kw_orig, 'ProbCondMatrix', accept_none=True
    )
    ...
    average_prob_log_prefix = get_text_value(
        blocked_spec_dict, kw_bw, 'AverageProbLogPrefix'
    )
    bw_zone_log_name = get_text_value(
        blocked_spec_dict, kw_bw, 'BlockedWellZoneLogName'
    )
    bw_job_name_list = get_list(blocked_spec_dict, kw_bw, 'BlockedWellJobNames')
    bias_weighting_dict = get_dict(
        blocked_spec_dict, kw_orig, 'BiasWeighting', accept_none=True
    )
    ...
    params['bw_job_list'] = bw_job_name_list
...
if __name__ == '__main__':
    ...
    blocked_wells_prob_log_names = ['Prob_F1', 'Prob_F2', 'Prob_F3', 'Prob_F4']
    well_list = [
        'W1',
        'W2',
        'W3',
