...
class DefineTruncationRule:
            ...
            while not finished:
                ...
                if line == '':
                    ...
                    if len(words) > 0 and words[0] != '#':
                        ...
                        if ruleType == 'Bayfill':
                            name = words[1]
                            truncStructureBayfill = (
                                self.__getTruncSettingsBayfillFromFile(words)
                            )
                            self.__tableBayfill[name] = truncStructureBayfill
                        elif ruleType == 'Cubic':
                            name = words[1]
                            truncStructureCubic = self.__getTruncSettingsCubicFromFile(
                                words
                            )
                            ...
                            truncStructureNonCubic = (
                                self.__getTruncSettingsNonCubicFromFile(words)
                            )
                            self.__tableNonCubic[name] = truncStructureNonCubic
                        elif ruleType == 'Overlay':
                            name = words[1]
                            overlayGroups = self.__getTruncSettingsOverlayFromFile(
                                words
                            )
    ...
    def writeFile(self, outputFileName):
        ...
        with open(outputFile, 'w', encoding='utf-8') as file:
            sortedDictionaryCubic = collections.OrderedDict(
                sorted(self.__tableCubic.items())
            )
            ...
            sortedDictionaryNonCubic = collections.OrderedDict(
                sorted(self.__tableNonCubic.items())
            )
            ...
            ALPHA_LIST_INDX = OverlayGroupIndices.ALPHA_LIST_INDX
            BACKGROUND_LIST_INDX = OverlayGroupIndices.BACKGROUND_LIST_INDX
            sortedDictionaryOverlay = collections.OrderedDict(
                sorted(self.__tableOverlay.items())
            )
            ...
            NAME_BG_INDX = CubicAndOverlayIndices.NAME_BG_INDX
            NAME_OL_INDX = CubicAndOverlayIndices.NAME_OL_INDX
            sortedDictionaryCubicAndOverlay = collections.OrderedDict(
                sorted(self.__tableCubicAndOverlay.items())
            )
            # for name, truncStructItemWithOverlay in self.__tableCubicAndOverlay.items():
            for (
                name,
                truncStructItemWithOverlay,
            ) in sortedDictionaryCubicAndOverlay.items():
                nameBG = truncStructItemWithOverlay[NAME_BG_INDX]
                nameOL = truncStructItemWithOverlay[NAME_OL_INDX]
                outString = (
                    'CubicAndOverlay' + ' ' + name + '  ' + nameBG + ' ' + nameOL
                )
                file.write(outString)
                file.write('\n')
            ...
            NAME_BG_INDX = NonCubicAndOverlayIndices.NAME_BG_INDX
            NAME_OL_INDX = NonCubicAndOverlayIndices.NAME_OL_INDX
            sortedDictionaryNonCubicAndOverlay = collections.OrderedDict(
                sorted(self.__tableNonCubicAndOverlay.items())
            )
            # for name, truncStructItemWithOverlay in self.__tableNonCubicAndOverlay.items():
            for (
                name,
                truncStructItemWithOverlay,
            ) in sortedDictionaryNonCubicAndOverlay.items():
                nameBG = truncStructItemWithOverlay[NAME_BG_INDX]
                nameOL = truncStructItemWithOverlay[NAME_OL_INDX]
                outString = (
                    'NonCubicAndOverlay' + ' ' + name + '  ' + nameBG + ' ' + nameOL
                )
                file.write(outString)
                file.write('\n')
    ...
    def __getTruncSettingsCubicFromFile(self, words):
        ...
        if words[3][2] == 'H':
            ...
            raise IOError(
                'Format error for word number 4 in file {}'.format(self.__inputFileName)
            )
    ...
    def getTruncationRuleObject(self, name):
        ...
        if itemBayfill is not None:
            ...
            ) = self.__background(itemBayfill, 'Bayfill')
            truncObj.initialize(
                mainFaciesTable,
                faciesInZone,
        ...
        if itemCubic is not None:
            if self.debug_level >= Debug.VERBOSE:
                print(
                    'Debug output: Create truncation object of type Cubic from truncation setting with name: {}'.format(
                        name
                    )
                )
            ...
            (
                ...
                gaussFieldsInZone,
                alphaFieldNameForBackGroundFacies,
            ) = self.__background(truncStructureList, 'Cubic')
            overlayGroups = None
        ...
        if itemCubicOverlay is not None:
            if self.debug_level >= Debug.VERBOSE:
                print(
                    'Debug output: Create truncation object of type Cubic with overlay facies from truncation setting with name: {}'.format(
                        name
                    )
                )
            ...
            if self.__checkOverlayFacies(faciesInZone, overlayGroups):
                gaussFieldsInZone, faciesInZone = (
                    self.__updateGaussFieldNamesandFaciesInZoneForOverlay(
                        overlayGroups, gaussFieldsInZone, faciesInZone
                    )
                )
                truncObj = Trunc2D_Cubic()
                truncObj.initialize(
        ...
        if itemNonCubic is not None:
            if self.debug_level >= Debug.VERBOSE:
                print(
                    'Debug output: Create truncation object of type NonCubic from truncation setting with name: {}'.format(
                        name
                    )
                )
            ...
            (
                ...
                gaussFieldsInZone,
                alphaFieldNameForBackGroundFacies,
            ) = self.__background(truncStructureList, 'NonCubic')
            overlayGroups = None
        ...
        if itemNonCubicOverlay is not None:
            if self.debug_level >= Debug.VERBOSE:
                print(
                    'Debug output: Create truncation object of type NonCubic with overlay facies from truncation setting with name: {}'.format(
                        name
                    )
                )
    ...
    def __updateGaussFieldNamesandFaciesInZoneForOverlay(
        self, overlayGroups, gaussFieldNameList, faciesNameList
    ):
        ...
        if overlayGroups is not None:
            for n in range(len(overlayGroups)):
                ...
                for i in range(len(alphaList)):
                    ...
                    if (
                        alphaName == gaussFieldNameList[0]
                        or alphaName == gaussFieldNameList[1]
                    ):
                        raise ValueError(
                            'Can not have gauss field names for overlay facies equal to {} or {}.'
    ...
    def addPolygonToTruncationRuleSettingsCubic(
        self, truncStructureCubic, faciesName, probFrac, L1, L2, L3
    ):
    ...
    def addTruncationRuleSettingsCubic(
        self, name, truncStructureSetting, replace=False
    ):
    ...
    def addPolygonToTruncationRuleSettingsNonCubic(
        self, truncStructureCubic, faciesName, angle, probFrac
    ):
    ...
    def addTruncationRuleSettingsNonCubic(
        self, name, truncStructureSetting, replace=False
    ):
    ...
    def addPolygonToAlphaList(
        self, alphaName, faciesName, probFrac=1.0, centerPoint=0.0, alphaList=None
    ):
    ...
    def addOverlayGroupSettings(
        self, alphaList, backgroundFaciesListForGroup, overlayGroups=None
    ):
    ...
    def addTruncationRuleSettingsCubicWithOverlay(
        self, name, nameBG, nameOL, replace=False
    ):
        ...
        try:
            ...
            raise KeyError(
                'Background facies truncation rule setting: {} is not defined'.format(
                    nameBG
                )
            )
        try:
            ...
            raise KeyError(
                'Overlay facies truncation rule setting: {} is not defined'.format(
                    nameOL
                )
            )

        # Check consistency between chosen background truncation rule setting and overlay facies truncation rule setting.
        if not self.__checkConsistencyBetweenBackGroundCubicAndOverlay(
            truncStructBG, overlayGroups
        ):
            raise ValueError(
    ...
    def addTruncationRuleSettingsNonCubicWithOverlay(
        self, name, nameBG, nameOL, replace=False
    ):
        """Add a truncation rule settings to the truncation rule settings dictionary
        using name as key. Name of background truncation rule and overlay truncation rule are input.
        ...
        # Check consistency between chosen background truncation rule setting and overlay facies truncation rule setting
        if not self.__checkConsistencyBetweenBackGroundNonCubicAndOverlay(
            truncStructBG, overlayGroups
        ):
            raise ValueError(
                'Specified background facies truncation setting {} '
    ...
    def __checkConsistencyBetweenBackGroundCubicAndOverlay(
        self, truncStructBG, overlayGroups
    ):
        # Check consistency between chosen background truncation rule setting and overlay facies truncation rule setting
        bgFacies = []
    ...
    def __checkConsistencyBetweenBackGroundCubicAndOverlayOld(
        self, nameBG, nameOL, truncStructBG, overlayGroups
    ):
        # Check consistency between chosen background truncation rule setting and overlay facies truncation rule setting
        bgFacies = []
        FACIES_NAME_INDX_BG = CubicPolygonIndices.FACIES_NAME_INDX
    ...
    def __checkConsistencyBetweenBackGroundNonCubicAndOverlay(
        self, truncStructBG, overlayGroups
    ):
        # Check consistency between chosen background truncation rule setting and overlay facies truncation rule setting
        bgFacies = []
    ...
    def removeTruncationRuleSettings(
        self, name, removeDependentBG=False, removeDependentOL=False
    ):
    ...
    def getListOfSettings(self, truncType, nBackgroundFacies=0, nOverlayFacies=0):
        if truncType == 'Cubic':
            ...
            settingsList = self.__appendToList(
                sortedDictionary, truncType, nBackgroundFacies, nOverlayFacies
            )
        elif truncType == 'NonCubic':
            sortedDictionary = collections.OrderedDict(
                sorted(self.__tableNonCubic.items())
            ...
            raise ValueError(f'Truncation type {truncType} is not defined.')
    ...
    def getListOfOverlaySettings(self, name):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise KeyError(
                        f'Truncation setting with name {name} is not defined.'
                    )
        ...
        if itemCubic is not None:
            ...
            for key, itemOverlay in sortedDictionary.items():
                if self.__checkConsistencyBetweenBackGroundCubicAndOverlay(
                    itemCubic, itemOverlay
                ):
            ...
            for key, itemOverlay in sortedDictionary.items():
                if self.__checkConsistencyBetweenBackGroundNonCubicAndOverlay(
                    itemNonCubic, itemOverlay
                ):
                    overlay_setting_list.append(key)
        elif itemBayfill is not None:
            sortedDictionary = collections.OrderedDict(
                sorted(self.__tableBayfill.items())
            )
            for key, item in sortedDictionary.items():
                overlay_setting_list.append(key)
        else:
    ...
    def __makeTruncationMapSubPlot(self, name, fig, Nrow, Ncol, indx):
        ...
        if self.debug_level >= Debug.ON:
            ...
            print(
                'Debug output: Number of facies polygons: ' + str(len(faciesPolygons))
            )
    ...
    def createAllBayfillPlots(self):
        sortedDictionaryBayfill = collections.OrderedDict(
            sorted(self.__tableBayfill.items())
        )
    ...
    def _create_overview_plot(self, plot_name, truncation_table):
        ...
        sorted_truncation_rule = collections.OrderedDict(
            sorted(truncation_table.items())
        )
...
def run_example():
    ...
    for faciesName, probFrac, L1, L2, L3 in polygons:
        rules.addPolygonToTruncationRuleSettingsCubic(
            truncStructureCubic, faciesName, probFrac, L1, L2, L3
        )
    ...
    for faciesName, probFrac, angle in polygons:
        rules.addPolygonToTruncationRuleSettingsNonCubic(
            truncStructureNonCubic, faciesName, angle, probFrac
        )
    ...
    overlayGroups = rules.addOverlayGroupSettings(
        alphaList, backgroundFaciesListForGroup
    )
    ...
    overlayGroups = rules.addOverlayGroupSettings(
        alphaList, backgroundFaciesListForGroup, overlayGroups
    )
    ...
    for nBackgroundFacies in range(1, 6):
        print(
            'List of settings for Cubic with background facies: {}'.format(
                nBackgroundFacies
            )
        )
        ...
        for nOverlayFacies in range(5):
            print(
                'List of settings for Cubic with background facies: {}   and overlay facies: {}'
                ''.format(nBackgroundFacies, nOverlayFacies)
            )

            settingsList = rules.getListOfSettings(
                'CubicAndOverlay', nBackgroundFacies, nOverlayFacies
            )
            for i in range(len(settingsList)):
                item = settingsList[i]
                key = item[0]
    ...
    for nBackgroundFacies in range(1, 6):
        print(
            'List of settings for NonCubic with background facies: {}'.format(
                nBackgroundFacies
            )
        )
        ...
        for nOverlayFacies in range(5):
            print(
                'List of settings for NonCubic with background facies: {}   and overlay facies: {}'
                ''.format(nBackgroundFacies, nOverlayFacies)
            )

            settingsList = rules.getListOfSettings(
                'NonCubicAndOverlay', nBackgroundFacies, nOverlayFacies
            )
            for i in range(len(settingsList)):
                item = settingsList[i]
                key = item[0]
    ...
    rules.removeTruncationRuleSettings(
        'NonCubic_1_with_overlay_1', removeDependentBG=True, removeDependentOL=True
    )
