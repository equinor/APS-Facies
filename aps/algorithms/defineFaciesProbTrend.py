...
class DefineFaciesProb(BaseDefineFacies):
    ...
    def _read_model_from_yml(self, debug_level: Debug = Debug.OFF):
        self._use_const_prob = get_bool_value(
            self._parent_dict, 'UseConstantProbFromVolumeFraction', True
        )
        if not self._use_const_prob:
            cond_table = get_dict(self._parent_dict, self._parent_kw, 'CondProbMatrix')
            conditional_prob_facies = get_cond_prob_dict(
                cond_table, self.selected_zone_numbers
            )
            ...
            if debug_level >= Debug.VERBOSE:
                ...
                for key, prob_value in sorted_by_zone_dict.items():
                    ...
                    print(
                        f'Zone: {zone_number}  P({facies_name}|{facies_name_in_real}) = {prob_value}'
                    )
    ...
    def calculate_facies_probability_parameter(self):
        ...
        if not self.use_const_prob:
            ...
            prob_index = self.calculate_probability_indices(
                code_names_facies, facies_names_in_real, facies_real_values
            )

            # probability matrix using indices corresponding to the lists facies_names and facies_names_in_real
            probabilities = self.calculate_probabilities(
                self.selected_zone_numbers, facies_names, facies_names_in_real
            )
            ...
            for f in range(num_facies):
                ...
                for zone_index, zone_number in enumerate(self.selected_zone_numbers):
                    ...
                    num_defined_cells, cell_index = (
                        getCellValuesFilteredOnDiscreteParam(zone_number, zone_values)
                    )
                    ...
                    probability_values[cell_index] = probabilities[
                        zone_index, f, p_index_array
                    ]
    ...
    @staticmethod
    def calculate_probability_indices(
        code_names_facies, facies_names_in_real, facies_real_values
    ):
    ...
    def calculate_probabilities(
        ...
        probabilities = np.zeros(
            (num_zones_selected, num_facies, num_facies_in_real), dtype=np.float32
        )
        for key in self.probability_matrix:
            (zone_number, facies_name, facies_name_in_real) = key
