...
PLUGIN_DIR := $(CODE_DIR)/$(PLUGIN_NAME)
...
build-gui: clean-plugin build-front-end gather-python-scripts auxillary-files
	cd $(CODE_DIR) && \
	zip -rm -9 $(PLUGIN_BIN) $(PLUGIN_NAME)

gather-python-scripts: copy-python-files
	cp $(UI.PY) $(PLUGIN_DIR)
	rm -rf $(PLUGIN_DIR)/src/unit_tests $(PLUGIN_DIR)/api
...
initWorkflow: autogenerated-constants links generate-workflow-files

init: initialize-python-environment dependencies autogenerated-constants links generate-workflow-files package.json
...
generate-workflow-files: create-workflow-dir
	$(PYTHON) $(BIN_DIR)/generate_workflow_blocks.py $(CODE_DIR)
