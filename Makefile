...
build-gui: clean-all libdraw2D.so resource-file ui-files set-autogenerated
	pyinstaller --onefile \
	            --clean \
	            --noconfirm \
	            --add-data="../README.md:." \
	            --add-data="$(EXAMPLES_FOLDER)/APS.xml:." \
	            --add-data="$(EXAMPLES_FOLDER)/rms_project_data_for_APS_gui.xml:." \
	            --add-data="$(CODE_DIR)/matplotlibrc:." \
...
set-version:
	sed -i -e "s|VERSION_HASH = '.*'|VERSION_HASH = '$(shell git rev-parse --short HEAD)'|g" $(PYTHON_LIBRARY_CONSTANT)

unset-version:
	sed -i -e "s|VERSION_HASH = '.*'|VERSION_HASH = ''|g" $(PYTHON_LIBRARY_CONSTANT)

set-path-to-library:
	sed -i -e "s|LIBRARY_FOLDER = '.*'|LIBRARY_FOLDER = '$(LIB_PREFIX)'|g" $(PYTHON_LIBRARY_CONSTANT)

unset-path-to-library:
	sed -i -e "s|LIBRARY_FOLDER = '.*'|LIBRARY_FOLDER = ''|g" $(PYTHON_LIBRARY_CONSTANT)

set-path-to-examples:
	sed -i -e "s|EXAMPLE_FOLDER = '.*'|EXAMPLE_FOLDER = '$(EXAMPLES_FOLDER)'|g" $(PYTHON_LIBRARY_CONSTANT)

unset-path-to-examples:
	sed -i -e "s|EXAMPLE_FOLDER = '.*'|EXAMPLE_FOLDER = ''|g" $(PYTHON_LIBRARY_CONSTANT)

set-autogenerated: set-version set-path-to-library set-path-to-examples

unset-autogrenerated: unset-version unset-path-to-examples unset-path-to-library

clean: unset-autogrenerated
	rm -rf $(BUILD_DIR) && \
