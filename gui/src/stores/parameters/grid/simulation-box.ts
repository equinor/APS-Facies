...
export const useParameterGridSimulationBoxStore = defineStore(
  ...
  () => {
    ...
    const _simulationBox = computed<RmsSimulationBoxSize | null>(() => {
      ...
      const relevantSimulationBoxes = simulationBoxes.value[gridModelStore.current.name]
      if (!relevantSimulationBoxes /* may be undefined */ ) return null
      return simulationBoxes.value[gridModelStore.current.name][rough.value ? 'true' : 'false'] ?? null
    })

    const size = computed<SimulationBoxSize>(() => {
      return _simulationBox.value?.size ?? {
        x: null,
        y: null,
        z: null,
      }
    ...
    const origin = computed<Coordinate2D>(() => {
      return _simulationBox.value?.origin ?? {
        x: null,
        y: null,
      }
    })
