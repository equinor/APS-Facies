...
import type { Facies, Polygon, Parent, GlobalFacies } from '@/utils/domain'
import {
  Bayfill,
  BayfillPolygon,
  NonCubic,
...
} from '@/stores/parameters/debug-level'
import type {
...
function getMandatoryNumericValue<T extends number, Obj extends object>(
  elem: Obj,
  prop: keyof Obj,
  ...
  if (guard && !guard(value))
    throw new Error(`'${prop as string}' has an illegal value`)
  return value as T
}
...
function getFacies(name: string, parent: Parent): Facies | undefined {
...
type ExpectedBayfillFaciesNames =
  | Exclude<BayfillFacies, 'Bayhead Delta' | 'Wave influenced Bayfill'>
  | 'BHD'
  | 'WBF'
...
function getSlantFactorKey(
  facies: SlantFactorFacies,
): ExpectedSlantFactorKeys | null {
  switch (facies) {
    case 'Floodplain':
      return 'SF'
    case 'Subbay':
...
function getAlphaField(name: string, parent: Parent): GaussianRandomField {
...
function getAlphaFields(
  ...
  return alphaFields
    .split(' ')
    .map((name: string): GaussianRandomField => getAlphaField(name, parent))
}
...
function makeOverlayPolygons(
  ...
  if ('OverLayModel' in container) {
    const groups = ensureArray(
      getMandatoryNodeValue(container, 'OverLayModel').Group,
    )
...
type TruncationRuleContentOverlay =
  | TruncationRuleContentNonCubic
  | TruncationRuleContentCubic
...
function makeCubicTruncationRule(
  ...
  ): (ProcessedProbabilityFraction | NodeTag)[] {
  ...
  function getChildren(
    ...
    const parents = flattened.reduce(
      (nodes, polygon) => {
        if ('@_id' in polygon) {
...
function extractTrend(
  container: ZoneModelGaussFieldContent['Trend'],
): TrendDescription {
...
function getTrend(
  gaussFieldFromFile: ZoneModelGaussFieldContent,
): Optional<Trend> {
...
function getCrossSection(parent: Parent): CrossSection | null {
...
function getMandatoryTextValue<T extends object>(
  element: T,
  keyword: keyof T,
): T[keyof T] | string {
...
type ProcessedProbabilityFraction = ProbabilityFraction &
  Attributes<{
...
function populatePreviewSettings(apsModelContainer: APSModelContent) {
  ...
  let currentRegionNumber = previewSettings['@_regionNumber']
    ? parseInt(previewSettings['@_regionNumber'], 10)
    : null
  if (currentRegionNumber === 0) {
    currentRegionNumber = null
  }
  if (currentZoneNumber >= 0) {
    const { zone, region } = zoneStore.byCode(
      currentZoneNumber,
      currentRegionNumber,
    )
    zoneStore.setCurrentId(zone.id)
    if (region) {
      useRegionStore().setCurrentId(region.id)
    }
  }
...
export const useModelFileLoaderStore = defineStore('model-file-loader', () => {
   ...
   */
  async function populateGUI(json: string, fileName: string): Promise<void> {
    const apsModelContainer = (JSON.parse(json) as ConvertedXMLContent)[
      'APSModel'
    ]
  ...
  async function populateGlobalFaciesList(
    ...
    if (mainFaciesTableFromFile === null)
      throw new Error(
        '<MainFaciesTable> is missing or empty from the model file',
      )
    await useParameterBlockedWellStore().select(
      mainFaciesTableFromFile['@_blockedWell'],
    )
    await useParameterBlockedWellLogStore().select(
      mainFaciesTableFromFile['@_blockedWellLog'],
    )
    ...
    for (const faciesContainer of mainFaciesTableFromFile.Facies) {
      ...
      if (!isNumber(code)) {
        throw new Error(
          `The global facies, '${name}' has an invalid number (${code})`,
        )
      } else if (code < 0) {
        throw new APSError(
          `The global facies, '${name}' has a negative code (${code})`,
        )
      }
  ...
  async function selectZonesAndRegions(
    ...
    if (regionToSetAsCurrent) {
      ...
      await Promise.all(regionsToSelect.map((region) => touchRegion(region)))
    }
  }
      ...
      for (const gaussFieldFromFile of zoneModel.GaussField) {
        add(
          new GaussianRandomField({
            ...
            seed:
              'SeedForPreview' in gaussFieldFromFile
  ...
  async function populateFaciesProbabilities(
    ...
    const { add, setConstantProbability, averageProbabilityCubes } =
      useFaciesStore()
    const { available: availableGlobalFacies } = useGlobalFaciesStore()
    for (const zoneModel of zoneModelsFromFile) {
      const useConstantProb = getBooleanValue(zoneModel, 'UseConstProb')
      const parent = getParent(zoneModel)
      for (const faciesModel of getMandatoryNodeValue(
        zoneModel,
        'FaciesProbForModel',
      ).Facies) {
        const facies = add(
          availableGlobalFacies.find(
            (obj): boolean => obj.name === getName(faciesModel),
          ) as GlobalFacies,
          parent,
        )

        setConstantProbability(facies.parentId, !!useConstantProb)
        if (useConstantProb) {
          const probability = getNumericValue(faciesModel, 'ProbCube')
          if (probability === null)
            throw new Error(
              `<UseConstProb> is set, but <ProbCube> is empty for Facies(name='${facies.name}', code=${facies.code})`,
            )
          facies.previewProbability = probability
        } else {
          const probabilityCube = getTextValue(
            faciesModel,
            'ProbCube',
          ) as ProbabilityCube | null
          if (probabilityCube === null)
            throw new Error(
              `<ProbCube> is not set for Facies(name='${facies.name}', code=${facies.code})`,
            )
  ...
  async function populateTruncationRules(
    ...
    const { available: availableTruncationRuleTemplateTypes } =
      useTruncationRuleTemplateTypeStore()
    for (const zoneModel of zoneModelsFromFile) {
      const parent = getParent(zoneModel)
  ...
  async function populateJobSettings(
    ...
    const jobSettingsParam = jobSettings(
      apsModelContainer,
      modelFileContainsFmuSettings,
    )
    ...
    fmuOptions.customTrendExtrapolationMethod =
      jobSettingsParam.customTrendExtrapolationMethod
    fmuOptions.onlyUpdateResidualFields =
      jobSettingsParam.onlyUpdateResidualFields
    ...
    useParametersMaxFractionOfValuesOutsideToleranceStore().setTolerance(
      jobSettingsParam.maxAllowedFractionOfValuesOutsideTolerance,
    )
    useParametersToleranceOfProbabilityNormalisationStore().setTolerance(
      jobSettingsParam.toleranceOfProbabilityNormalisation,
    )
    useParameterTransformTypeStore().select(jobSettingsParam.transformType)
    useParameterDebugLevelStore().select(jobSettingsParam.debugLevel)
  }
  return {
    populateGUI,
  }
