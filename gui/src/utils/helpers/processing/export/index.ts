...
function addJobSettings({ rootState}: Context, doc: Document, parentElement: HTMLElement): void {
  ...
  if(fmuMode === "FIELDS"){
    ...
    addKeyword(doc, 'UseResidualFields', useResidualFields, fmuSettingsElement, true)
    addKeyword(doc, 'UseNonStandardFMU', useNonStandardFMUSettings, fmuSettingsElement, true)
  }
...
function addTruncationRuleNonCubic (context: Context, doc: Document, parent: Parent, truncRule: NonCubic, truncRuleElem: HTMLElement): void {
  function handleBackgroundPolygons (backGroundModelElem: HTMLElement): void {
    ...
    truncRule.backgroundPolygons.forEach((polygon): void => {
      ...
      // angle element
      const kwValue = `APS_${parent.zone.code}_${parent.region ? parent.region.code : 0}_TRUNC_NONCUBIC_POLYNUMBER_${polygon.order + 1}_ANGLE`
